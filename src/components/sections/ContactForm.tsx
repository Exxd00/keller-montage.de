...
export function ContactForm({ variant = "default" }: { variant?: "default" | "minimal" }) {
  ...
  // Get GCLID and source for tracking
  const getTrackingData = useCallback(() => {
    if (typeof window === "undefined") return { gclid: "-", source: "Direct" };
    ...
    if (gclid && gclid !== "-") {
      ...
      else if (referrer.includes("bing.") || referrer.includes("yahoo.")) source = "Organic";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      submitData.append("gclid", trackingData.gclid);
      submitData.append("source", trackingData.source);
